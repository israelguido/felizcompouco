...
<?php foreach ($this->items as $i => $item) : ?>
	<div class="row<?php echo $i % 2; ?>">
            ...
            <?php if ($params->get('link_titles')): ?>
                <?php $catslug = !empty($item->catslug) ? $item->catslug : ($item->catid . (!empty($item->category_alias) ? ':' . $item->category_alias : '')); ?>
                <a href="<?php echo JRoute::_(ContentHelperRoute::getArticleRoute($item->slug, $catslug)); ?>">
...
<dt></dt>
		<dd class="category-name">
			<?php	$title = $this->escape($item->category_title);
					$catslug = !empty($item->catslug) ? $item->catslug : ($item->catid . (!empty($item->category_alias) ? ':' . $item->category_alias : ''));
