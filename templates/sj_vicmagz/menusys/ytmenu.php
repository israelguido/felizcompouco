	...
	class YtMenu extends YtObject{
		...
		function __construct($item=null, $params=null){
			...
			if (!isset($item)){
				...
				// Joomla 4+: MenuItem::$params is protected
				$itemParams = method_exists($item, 'getParams') ? $item->getParams() : $item->params;
				$this->params->bind(method_exists($itemParams, 'toArray') ? $itemParams->toArray() : (array) $itemParams);
		...
		public function getClass(){
			$liClass = '';
			$mega_menu_class = $this->getParams()->get('ytmenu_class');
		...
		public function getContent($tpl='default'){
			$menustyle = strtolower($this->getParams()->get('menustyle', 'basic'));
			$overload = $this->getParams()->get('basepath') . J_SEPARATOR . 'class' . J_SEPARATOR . 'common' . J_SEPARATOR . 'html' . J_SEPARATOR . $menustyle .J_SEPARATOR . $tpl . '.php';
			$template = $this->getParams()->get('basepath') . J_SEPARATOR . 'class' . J_SEPARATOR . $menustyle . J_SEPARATOR . 'tmpl' . J_SEPARATOR . $tpl . '.php';
		...
		private function _getAssetsPath(){
			$menustyle = strtolower($this->getParams()->get('menustyle', 'basic'));
			$abspath = $this->getParams()->get('basepath') . J_SEPARATOR . 'class' . J_SEPARATOR . $menustyle . J_SEPARATOR . 'assets' . J_SEPARATOR;
			$abspath = realpath($abspath);
			!empty($abspath) or die($this->getParams()->get('basepath') . ' does not exits. Please kindly set basepath for menusys');
		...
		public function getLinkInMobile($level){
			...
			if ($itemtype=='menulink' && false!=$this->_aliasitem){
				...
				$tmp =& $this;
				$tmp->id = $this->getParams()->get('aliasoptions');
			} else {
				$tmp =& $this;
			}

			$menu_title =  htmlspecialchars($this->title);
			$icon_menu = trim((string) $tmp->getParams()->get('menu-anchor_css', ''));
			$icon_menu = ($icon_menu!='')?'<i class="'.$icon_menu.'"></i>':'';

			$title_attr = trim((string) $tmp->getParams()->get('menu-anchor_title', ''));
			$title_attr = ($title_attr !== '') ? $title_attr : htmlspecialchars($this->title);
			...
			switch($itemtype){
				...
				case 'url':
					...
					break;
				default:
					$sef = (int) JFactory::getApplication()->get('sef', 1);
					if ($sef) {
			...
			if (!empty($anchor_href)){
				// Handle SSL links
				$iSecure = $tmp->getParams()->get('secure', 0);
		...
		public function getLink(){
			...
			$menu_image = $tmp->getParams()->get('menu_image');
			...
			$menu_desc  = $this->getParams()->get('ytext_desc', null);
			...
			if (!empty($anchor_href)){
				...
				switch ($tmp->browserNav){
					...
					case 2:
						// window.open
						$attribs = 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,'.$this->getParams()->get('window_open');
		...
		public function getSubmenuWidth(){
			$subWidth = $this->getParams()->get('ytext_submenuwidth');
			$subStyle = "";
		...
		public function haveMegaContent(){
			$haveContent = false;
			$contentType = $this->getParams()->get('ytext_contenttype');
			switch($contentType){
				default:
				case 'menu':
				case 'megachild':
					...
					break;
				case 'mod':
					$ytext_modules     = $this->getParams()->get('ytext_modules','');
					$haveContent = !empty($ytext_modules) && count($this->loadModules())>0;
					
					break;
				case 'pos':
					$ytext_positions   = $this->getParams()->get('ytext_positions', '');
		...
		public function loadModules(){
			if (!isset($this->__modules)){
				$item_content_type = $this->getParams()->get('ytext_contenttype', 'menu');
				$ytext_modules     = $this->getParams()->get('ytext_modules',   '');
				$ytext_positions   = $this->getParams()->get('ytext_positions', '');
				$user = JFactory::getUser();
				$modules = array();
				...
				if ($item_content_type=='mod'){
					...
					if (!empty($posname)){
						foreach ($posname as $pos){
							if ($pos == $this->getParams()->get('exclude_positions', false)){
								continue;
							}
							$db = JFactory::getDbo();
		...
		public function getMegaCols(){
			$ytext_cols = $this->getParams()->get('ytext_cols', 1);
			$ytext_width = $this->getParams()->get('ytext_width', 0);
			$ytext_columns_width = $this->getParams()->get('ytext_colwidth', '');
			...
			//if ($t-$ytext_width) $this->getParams()->set('ytext_width', $t);
			return $cols;
		}
	}
}