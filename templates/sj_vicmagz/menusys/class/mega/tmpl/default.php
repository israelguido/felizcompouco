...
if ( $this->isRoot() ){
	...
	$menucssid = $this->getParams()->get('menustyle') . 'navigator' . $this->getParams()->get('cssidsuffix');
	...
	$timeOut   	= $this->getParams()->get('moofxtimeout', '250');
	$transition = $this->getParams()->get('moofx', 'linear');
					...
					<div class="mega-col<?php echo $addclass;?><?php echo count($mega_cols)>1?' more':' one'; ?>" <?php echo $styleWidth; ?>>
						...
						if($item_content_type=='menu'){
							...
							foreach($cm[$cmi] as $k => $child){								
								$child->addClass($sublevelClass);
								$child->getParams()->set('styleofcolumn', $styleWidth);
