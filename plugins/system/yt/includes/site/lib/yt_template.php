...
abstract class YtFrameworkTemplate {
	...
	function __construct ($template = null, $cookie = array()) {
		$this->_tpl = $template;
	...
	*/
	...
	function getLogo(){
		...
		?>
			...
			<a class="logo" href="index.php" title="<?php echo $app->get('sitename'); ?> ">
				<img data-placeholder="no" src="<?php echo $url; ?>" alt="<?php echo $app->get('sitename'); ?>" style="<?php echo $logoWidth. $logoHeight  ?>"  />
			</a>
           
        <?php
		else:
            $logoText = (trim($this->getParam('logoText'))=='') ? $app->get('sitename') : $this->getParam('logoText');
			...
			<div class="logo-text" style="<?php echo $logoWidth. $logoHeight  ?>">
				<a class="site-text" href="index.php" title="<?php echo $app->get('sitename'); ?>"><?php echo $logoText; ?></a>
	...
	function renPositionsGroup($position, $type='', $special = null){
		...
		if(($position['group'] ?? '') == 'main'){
			$prefx = is_array($special) ? ($special['mainprefix'] ?? '') : '';
		}else{
			$prefx = '';
		}
		
		// Element class — keep configured grid class (don't force full-width for single positions in a group)
		if (isset($position['class']) && $position['class'] != '') {
