	...
	class YTTemplateTools_Image {
		...
		public function getOutputFile($renew=false){
			if(is_null($this->oFile) || $renew){
				...
				$imageType = $this->getOutputImageType();
				if ($imageType === null || $imageType === false) {
					$imageType = IMAGETYPE_JPEG;
				}
				$this->oFile = $cache . md5($serialize) . image_type_to_extension((int) $imageType);
			}
			return $this->oFile;
		}
		...
		public function getOutputImageType(){
			...
			if ($this->oImageType === null || $this->oImageType === false) {
				$this->oImageType = IMAGETYPE_JPEG;
			}
		...
		public function setBackground($color){
			...
			if (is_array($color)){
				...
				if (preg_match('/^#?([a-f0-9]{3}|[a-f0-9]{6})$/i', trim($color))){
