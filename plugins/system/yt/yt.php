...
class plgSystemYt extends JPlugin {
	...
	public function onBeforeCompileHead(){
		...
		if($app->isClient('site') && $layout == null ){
			unset($doc->_scripts[JURI::root(true) .'/media/jui/js/bootstrap.min.js']);
		}
		
		if($app->isClient('site') && !$k2detail && !$config_module && $layout== null && !$comMedia &&  $disMootool && !$registration) {
	...
	function onAfterRender() {
		...
		//  Minify
		if($app->isClient('site') && $document->_type == 'html' && !($option == 'com_content' && $task =='edit')){
