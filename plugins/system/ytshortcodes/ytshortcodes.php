...
// Joomla 6 legacy aliases for YT/SJ
...
class plgSystemYtshortcodes extends JPlugin{
	...
	public function onContentPrepare($context, &$article, &$params, $page=0){
		// Skip in CLI (e.g. finder:index) — shortcodes call JHtml::stylesheet which needs a Document
	...
	public function onBeforeCompileHead(){
		...
		if(( $app->isClient('site') && isset($valRequest['view']) && $valRequest['view'] == 'item' && isset($valRequest['task']) && $valRequest['task'] == 'edit') || ($app->isClient('site') && isset($valRequest['view']) && $valRequest['view'] == 'form' && isset($valRequest['layout']) && $valRequest['layout'] == 'edit') || $app->isClient('administrator') || $valRequest['option'] == 'com_config' ||  $valRequest['option'] == 'com_users' || $valRequest['view'] == 'registration'){
		...
		// include Jquery Joomla25 (skip on modern Joomla)
		$version = new JVersion();
		$release = property_exists($version, 'RELEASE') ? $version->RELEASE : implode('.', array_slice(explode('.', $version->getShortVersion()), 0, 2));
		if($this->params->get('show_sjjquery', 0)==1 && $release=='2.5' ){
	...
	public function addBtnShortCodes()
	{
		...
		$version = new JVersion();
		$release = property_exists($version, 'RELEASE') ? $version->RELEASE : implode('.', array_slice(explode('.', $version->getShortVersion()), 0, 2));
		if(version_compare($release, '3.5', '<') ){
